"""
Format C handler for month 10.2568.xlsx

Structure:
- 41 columns
- Header at row 3 (skiprows=3)
- Has ลำดับ column at position 0, so ID is at column 1, Name at column 2
- Single values (no halves to sum)
- Columns are in DIFFERENT ORDER than Format A/B - need remapping
...
class AbsenceFormat10(BaseFormatHandler):
    """Handler for Format C (month 10)."""

    def __init__(self):
        self.config = FORMAT_CONFIGS['C']
    ...
    def extract_employees(self, filepath: str) -> List[Dict[str, Any]]:
        """
        Extract employee data from Format C Excel file.

        Column mapping (reordered to match standard 17 types):
            [0] Work Days = col 7 (วันที่มาทำงาน)
            [1] Absent = col 17 (ขาดงาน)
            [2] Personal Leave = col 15 (ลากิจ)
            [3] Sick w/Cert = col 13 (ลาป่วย+ใบรพ)
            [4] Sick w/o Cert = col 14 (ลาป่วยไม่มีใบรพ)
            [5] Maternity = col 16 (ลาคลอด)
            [6] Late Grace = col 21 (ลืมสแกนนิ้ว)
            [7] Late Penalty = col 22 (เข้างานช้า)
            [8] OT Leave = col 20 (ลาโอที)
            [9] Suspension = col 18 (พักงาน)
            [10] Annual Leave = col 8 (ลาพักร้อน)
            [11] OT 2.5hr = col 9 (ทำโอที)
            [12] OT >2.5hr = col 10 (โอทีที่เกินเวลาปกติ)
            [13] Holiday Work = col 11 (ทำงานวันหยุด)
            [14] Holiday OT = col 12 (โอทีวันหยุด)
            [15] Night Shift = col 27 (วันที่เข้ากะดึก)
            [16] Multi-Machine = col 28 + col 29 (เครื่องคู่ x40 + x60)
        """
